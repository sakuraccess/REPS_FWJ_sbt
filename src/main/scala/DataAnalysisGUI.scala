...
object DataAnalysisGUI extends SimpleSwingApplication {
  def top: MainFrame = new MainFrame {
    ...
    private val tabs = new TabbedPane {
      pages += new TabbedPane.Page("Overview", createTabPanel("Combined Overview of Power Generation", "./charts/overviewChart.png"))
      pages += new TabbedPane.Page("Solar", createTabPanel("Solar Power Generation in last 24h", "./charts/solarChart.png"))
      pages += new TabbedPane.Page("Wind", createTabPanel("Wind Power Generation in last 24h", "./charts/windChart.png"))
      pages += new TabbedPane.Page("Hydro", createTabPanel("Hydro Power Generation in last 24h", "./charts/hydroChart.png"))
    }
    
    contents = tabs
    
    def createTabPanel(description: String, imagePath: String): Component = {
      val panel = new BoxPanel(Orientation.Horizontal) {
        ...
        contents += new ScrollPane(makeControlsPanel())
      }
      panel
    }
    
    private def makeControlsPanel(): GridBagPanel = {
        ...
        reactions += {
          case ButtonClicked(`searchButton`) =>
            ...
            if (validateDate(startDate) && validateDate(endDate)) {
              val results = analyzeData(startDate, endDate, timeSpan)
...
//              resultsArea.text = "Invalid date format. Please enter the date in the format 'DD/MM/YYYY'.\nFor example, enter '12/04/2024' for April 12, 2024."
    ...
    def validateDate(dateStr: String): Boolean = {
    ...
    private def analyzeData(startDate: String, endDate: String, timeSpan: String): (Double, Double, Double, Double, Double) = {
